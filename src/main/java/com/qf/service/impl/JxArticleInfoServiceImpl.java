...
public class JxArticleInfoServiceImpl implements JxIArticleService {
    ...
    public Map getInfoMap(int articleId) {
        ...
        hashMap.put("articleId", map2 == null ? null : map2.get("articleId"));
        return hashMap;
    }
    ...
    @Override
    public List<JxTag> getAllTag() {
        return jxArticleInfoMapper.selectAllTag();
    }

    @Override
    public int[] getArticleInfoIdByTagName(String[] tagNames) {
        List<String> list = new ArrayList<>(Arrays.asList(tagNames));
        return jxArticleInfoMapper.selectArticleInfoIdByTagName(list);
    }
}
